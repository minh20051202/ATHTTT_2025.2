...
const ATTACK_TYPES = [
  {
    ...
    label: "1. MITM",
    auth: "both",
    desc: "Proxy intercepts credentials in transit via TLS downgrade or inspection",
  },
  {
    key: "replay",
    label: "2. Token Replay",
    auth: "both",
    desc: "Reuse captured bearer token — OAuth2 succeeds, ZKP blocked by single-use token",
  },
  {
    key: "nonce-reuse",
    label: "3. Nonce Reuse Attack",
    auth: "zkp",
    desc: "Schnorr signature broken if nonce r is reused — secret key x algebraically recovered",
  },
];
...
function ForensicsReadout({
  result,
  capturedOauth2Token,
  capturedZkpChallenge,
  capturedZkpProof,
}) {
  ...
  const mitmFlow = result.details?.mitm_flow;
  const tamperedRequest = result.details?.tampered_request;
  const forwardedResult = result.details?.forwarded_result;
  ...
  const interceptedChallenge =
    result.details?.intercepted_data?.captured_challenge || capturedZkpChallenge;
  const interceptedProof =
    result.details?.intercepted_data?.captured_proof || capturedZkpProof;
  const capturedCredential = result.auth_type === "oauth2"
    ? capturedOauth2Token
    : interceptedChallenge || interceptedProof;
      ...
      >
        ...
        {mitmFlow && (
          <div>
            ...
            >
              {mitmFlow.map((step, index) => `${index + 1}. ${step}`).join("\n")}
            </pre>
          </div>
        )}
        ...
        {tamperedRequest && (
          <div>
            ...
            >
              {JSON.stringify(tamperedRequest, null, 2)}
            </pre>
          </div>
        )}
          ...
          <div>
            ...
            <pre
              style={{
                margin: 0,
                padding: "10px",
                background: result.success ? "rgba(239,68,68,0.06)" : "rgba(52,211,153,0.06)",
                border: result.success ? "1px solid rgba(239,68,68,0.3)" : "1px solid rgba(52,211,153,0.3)",
                color: "var(--terminal-text)",
                fontSize: "10px",
                overflowX: "auto",
                fontFamily: "var(--font-mono)",
            ...
            >
              {JSON.stringify(forwardedResult, null, 2)}
            </pre>
          </div>
        )}

        {result.auth_type === "zkp" &&
          result.details?.attack_logic &&
          result.details?.attack_logic?.nonce_r_reused && (
            <div>
              <div
                style={{
                  color: "var(--terminal-accent)",
            ...
            <div>
              ...
              >
                {result.auth_type === "oauth2"
                  ? `BEARER_TOKEN:\n${capturedOauth2Token}`
                  : [
                      interceptedChallenge
                        ? `ZKP_CHALLENGE_TOKEN:\n${interceptedChallenge}`
                        : null,
                      interceptedProof ? `ZKP_PROOF:\n${interceptedProof}` : null,
                    ].filter(Boolean).join("\n\n")}
              </pre>
            </div>
          )}
        ...
        <div>
          ...
          <div style={{ color: "var(--terminal-text)", fontSize: "12px" }}>
            {vulnerability}
          </div>
        </div>
      </div>
    ...
    >
      {steps.map((step, i) => (
        <div key={i} style={{ fontSize: "11px", display: "flex", gap: "10px" }}>
          ...
          <span style={{ color: "var(--terminal-text)", fontWeight: 700 }}>
            {step.label}
          </span>
...
function CompareCard({
  ...
  const oauth2Vulnerable =
    oauth2Results && Object.values(oauth2Results).some((r) => r?.success);
  const zkpVulnerable =
    zkpResults && Object.values(zkpResults).some((r) => r?.success);
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
        ...
        >
          ...
          >
            {oauth2Results ? (
              <>
                <StatusLED
                  status={oauth2Vulnerable ? "vulnerable" : "protected"}
                />
        ...
        >
          ...
          >
            {zkpResults ? (
              <>
                <StatusLED
                  status={zkpVulnerable ? "vulnerable" : "protected"}
                />
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {oauth2Results
            ? Object.entries(oauth2Results).map(
                ([key, result]) =>
                  result && (
                    <div key={key}>
                      <div
                      ...
                      <ForensicsReadout
                        result={result}
                        capturedOauth2Token={capturedOauth2Token}
                        capturedZkpChallenge={capturedZkpChallenge}
                        capturedZkpProof={capturedZkpProof}
                      />
                    </div>
                  ),
              )
...
export default function AttackPanel() {
  ...
  const zkpChallenge = resultsByAuth?.zkp?.auth_info?.zkp_token;
  const zkpProof = resultsByAuth?.zkp?.auth_info?.proof;
  const zkpToken = zkpChallenge || zkpProof;
  ...
  const executeAttack = useCallback(async () => {
    ...
    if (targetAuth === "zkp" && attackType !== "nonce-reuse" && !zkpHasToken)
      return;
    ...
    setAttackSteps([
      {
        label: "SCANNING_TRAFFIC",
        detail: "Sniffing auth channels...",
        success: true,
      },
    ]);
    ...
    setAttackSteps((prev) => [
      ...prev,
      {
        label: "INJECTING_EXPLOIT",
        detail: `${selectedAttack?.label}`,
        success: true,
      },
    ]);
    ...
    try {
      ...
      if (targetAuth === "oauth2" || targetAuth === "both") {
        if (oauth2HasToken) {
          ...
          oauth2Res = {
            success: false,
            message:
              "SKIPPED — OAuth2 agent has not sent a request yet. Chat with OAuth2 Agent first to capture token.",
      ...
      if (targetAuth === "zkp" || targetAuth === "both") {
        if (attackType === "nonce-reuse") {
          ...
          zkpRes = await apiFn("zkp", zkpToken, agentId, zkpProof);
        } else if (targetAuth === "both") {
          zkpRes = {
            success: false,
            message:
              "SKIPPED — ZKP agent has not sent a request yet. Chat with ZKP Agent first to capture proof.",
      ...
      setAttackSteps((prev) => [
        ...prev,
        { label: "INTERNAL_ERROR", detail: err.message, success: false },
      ]);
      setAttackPhase("done");
    }
  }, [
  ...
  const handleKeyDown = (e) => {
    ...
    if (e.key === "ArrowDown") {
      ...
      e.preventDefault();
      nextIndex =
        (currentIndex - 1 + ATTACK_TYPES.length) % ATTACK_TYPES.length;
    } else if (e.key === "Enter") {
      e.preventDefault();
      handleExecute();
      return;
  ...
  const selectedTargetAuth =
    ATTACK_TYPES.find((a) => a.key === attackType)?.auth || "oauth2";
  const canExecute =
    selectedTargetAuth === "both"
      ? oauth2HasToken || zkpHasToken
      : selectedTargetAuth === "oauth2"
        ? oauth2HasToken
        : attackType === "nonce-reuse"
          ? true // standalone demo with own vulnerable token
          : zkpHasToken;
      ...
      <div className="mission-control">
          ...
          >
            {ATTACK_TYPES.map(({ key, label, auth }) => (
              ...
              >
                ...
                <div style={{ fontSize: "9px", opacity: 0.7, fontWeight: 800 }}>
        ...
        >
          <button
            ...
            disabled={
              !canExecute || (attackPhase !== "idle" && attackPhase !== "done")
            }
          >
            Initialize Strike
          </button>
        ...
        {attackPhase === "done" && (
          ...
          >
            ...
            {attackResult && (
              <ForensicsReadout
                result={attackResult}
                capturedOauth2Token={oauth2Token}
                capturedZkpChallenge={zkpChallenge}
                capturedZkpProof={zkpProof}
              />
            )}
            {compareResult && (
              <div
              ...
              >
                <div>
                  ...
                  {compareResult.oauth2 && (
                    <ForensicsReadout
                      result={compareResult.oauth2[attackType]}
                      capturedOauth2Token={oauth2Token}
                      capturedZkpChallenge={zkpChallenge}
                      capturedZkpProof={zkpProof}
                    />
                ...
                <div>
                  ...
                  {compareResult.zkp && (
                    <ForensicsReadout
                      result={compareResult.zkp[attackType]}
                      capturedOauth2Token={oauth2Token}
                      capturedZkpChallenge={zkpChallenge}
                      capturedZkpProof={zkpProof}
                    />
